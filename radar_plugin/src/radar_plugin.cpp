...
void RadarPlugin::imageLongCB(const ConstLogicalCameraImagePtr &_msg) {
    ...
    for (int i=0; i<_msg->model_size() && this->objCount<64; ++i) {
        ...
        try {
            this->_listener.waitForTransform(this->radarFrame, "/base_link", ros::Time::now(), ros::Duration(1.0));
            this->_listener.lookupTransform(this->radarFrame, "/base_link", ros::Time(0), this->_transform);
        }
        catch (tf::TransformException ex) {
            continue;
        }
        ...
        tf::Vector3 transformVec = this->_transform.getOrigin();
        ignition::math::Vector3d baseTransformVec(transformVec.getX(), transformVec.getY(), transformVec.getZ());
        ignition::math::Vector3d modelPoseVec = modelRelativeVec - baseTransformVec;
